...
async function processInlineGalleries(content) {
  ...
  await Promise.all(albumIds.map(async id => {
    const [album, photos] = await Promise.all([
      db1(`SELECT * FROM gallery_albums WHERE id=? AND is_active=1`, [id]),
      db(`SELECT * FROM gallery_photos WHERE album_id=? ORDER BY sort_order ASC, id ASC`, [id]),
    ]);
  ...
  return content.replace(regex, (match, id) => {
    ...
    return `<div class="gt-article-gallery"><div class="gt-article-gallery-header"><i class="fa fa-images"></i> ${g.album.title} <span style="font-weight:400;color:var(--gray-500);margin-left:.4rem">${n} photo${n !== 1 ? 's' : ''}</span></div><div class="gt-bento-gallery" data-photos="${photosJson}"><div class="gt-bento-grid" data-sq="${squareCnt}">${tiles}</div></div></div>`;
  });
}
...
exports.blogPost = async (req, res) => {
  ...
  const allEmbeds = collectAllEmbeds(post.content, post.content_hi, post.content_te);
  post.content    = await processInlineGalleries(syncGalleries(post.content,    allEmbeds));
  post.content_hi = await processInlineGalleries(syncGalleries(post.content_hi, allEmbeds));
  post.content_te = await processInlineGalleries(syncGalleries(post.content_te, allEmbeds));
